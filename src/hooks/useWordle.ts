...
const useWordle = (solution: String) => {
  ...
  const [guesses, setGuesses] = useState<GuessObject[][]>([]); // each guess is an array
  ...
  const addNewGuess = (formattedGuess: GuessObject[]) => {
    if (currentGuess === solution) {
      setIsCorrect(true);
    }
    setGuesses((prevGuesses) => {
      let newGuesses: GuessObject[][] = [...prevGuesses];
